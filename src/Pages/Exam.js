    ...
    <>
      ...
      >
        {exam ? (
          <Grid
            container
            sx={{ bgcolor: "white", borderRadius: 3, m: 2, maxWidth: 700 }}
          >
            <Grid item xs={12} p={3}>
              <Grid container>
                <Grid item xs={12} md={6}>
                  ...
                  <Typography sx={{ fontSize: 20, color: "#757575" }}>
                    {exam.examName}
                  </Typography>
                </Grid>
                <Grid
            ...
            >
              <ExamSVG style={{ maxWidth: "300px", height: "auto" }} />
            </Box>
            <Grid item xs={12} sx={{ textAlign: "center" }}>
              ...
              {!timer.error && (
                ...
                >
                  <Box sx={timerStyles}>
                    {timer.days}{" "}
                    <Typography>Day{timer.days > 1 && "s"}</Typography>
                  </Box>
                  <Box sx={timerStyles}>
                    {timer.hours}{" "}
                    <Typography>Hour{timer.hours > 1 && "s"}</Typography>
                  </Box>
                  <Box sx={timerStyles}>
                    {timer.minutes}{" "}
                    <Typography>Minute{timer.minutes > 1 && "s"}</Typography>
                  </Box>
                  <Box sx={timerStyles}>
                    {timer.seconds}{" "}
                    <Typography>Second{timer.seconds > 1 && "s"}</Typography>
                  </Box>
                </Stack>
              )}
              {currentTime < endTime && (
